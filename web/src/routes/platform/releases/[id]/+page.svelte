...
<OperatorGuard>
	...
	{#if release}
		...
		<section class="card evidence"><div><span class="muted">Image digest</span><code>{release.image_digest}</code></div><div><span class="muted">Manifest digest</span><code>{release.manifest_digest}</code></div><div><span class="muted">Source commit</span><code>{release.source_commit}</code></div></section>

...
export async function signIn(returnTo:string,stepUp=false){const verifier=random(),state=random();sessionStorage.setItem(ATTEMPT,JSON.stringify({verifier,state,returnTo}));const query=new URLSearchParams({response_type:'code',client_id:CLIENT_ID,redirect_uri:REDIRECT_URI,scope:'openid profile email',state,code_challenge:await challenge(verifier),code_challenge_method:'S256'});if(stepUp){query.set('prompt','login');query.set('max_age','0')}location.assign(`${ISSUER}/oidc/authorize?${query}`)}
export async function stepUp(returnTo:string){return signIn(returnTo,true)}
